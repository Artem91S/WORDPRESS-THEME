...
const clickAction = function(event){
    ...
    buttonDiscription.forEach(element => {
        element.classList.remove("click-on-button");
        if(element.getAttribute("show-info") === nameOfClick){
            element.classList.add("click-on-button")
        }
    });
    nameOfButtons.forEach(buttons => {
        buttons.classList.remove("clikc-button-color");
        if(buttons.getAttribute("data-name") === nameOfClick){
            buttons.classList.add("clikc-button-color")
        }
    });
}
...
arr.forEach(key=>{
    ...
    <div class="creat-design">
    <div class="icons">
    <svg class="combined" width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    </svg><span class="squer"><span></span></span></div><div><h4>creative design</h4>
    <p>${key.name}</p>
    </div>
    </div>
...
function showPhoto(event) {
    ...
    if(ourWorkClick == "all" && counter < 13){
...
else{element.classList.remove("block-wokers")}
});
}